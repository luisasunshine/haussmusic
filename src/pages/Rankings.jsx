    ...
    <div className="min-h-screen pb-40 lg:pb-32">
        ...
        <div className="relative px-4 lg:px-6 xl:px-8 pt-6 lg:pt-8 pb-4 lg:pb-6">
          <div className="flex items-center gap-3 lg:gap-4">
            <motion.div
              ...
              className="w-12 lg:w-16 h-12 lg:h-16 rounded-2xl bg-gradient-to-br from-[#d8d8e2] to-[#f4f4f7] flex items-center justify-center shadow-lg shadow-[#d8d8e2]/30"
            >
              <Flame className="w-6 lg:w-8 h-6 lg:h-8 text-white" />
            </motion.div>
            <div>
              <h1 className="text-2xl lg:text-4xl font-bold v-chrome-text v-display">VELVET HITS</h1>
              <p className="text-sm lg:text-base text-[#9a9aa6]">O top 100 de cada categoria, atualizado ao vivo</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 lg:px-6 xl:px-8">
        <Tabs defaultValue="albums" className="w-full">
          <TabsList className="bg-[#101014] border border-[#1c1c22] mb-4 w-full grid grid-cols-4 p-1 rounded-xl">
            {tabConfigs.map(tab => (
              <TabsTrigger key={tab.value} value={tab.value} className="data-[state=active]:bg-[#d8d8e2]/20 data-[state=active]:text-[#f4f4f7] text-[11px] sm:text-sm lg:text-base flex items-center gap-1.5 lg:gap-2 rounded-lg">
          ...
          {tabConfigs.map(tab => (
            <TabsContent key={tab.value} value={tab.value}>
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-4 pt-2">
                <h2 className="text-lg lg:text-xl font-bold v-chrome-text">{tab.activeLabel}</h2>
                <p className="text-xs text-velvet-faint mt-0.5">Top {tab.list.length} de {TOP_N}</p>
              </motion.div>
              ...
              <div className="space-y-1">
                ...
                {tab.list.length === 0 && (
                  <div className="text-center py-12">
                    <div className="w-16 h-16 rounded-full bg-[#101014] flex items-center justify-center mx-auto mb-4">
                      <Flame className="w-8 h-8 text-[#1c1c22]" />
                    </div>
                    <h3 className="text-lg font-bold text-white mb-2">Nada por aqui ainda</h3>
                    <p className="text-[#9a9aa6] text-sm">Assim que algo bombar, aparece aqui.</p>
                  </div>
                )}
              </div>
            </TabsContent>
