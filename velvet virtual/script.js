...
async function loadMagazine() {
  ...
  try {
    ...
    book.addEventListener('pointerdown', (event) => {
      ...
      drag = { startX: event.clientX, startY: event.clientY, width: rect.width, page: pageElements[current], delta: 0, pointerId: event.pointerId, axis: null, locked: current === pages.length - 1 };
    });
    book.addEventListener('pointermove', (event) => {
      ...
      if (!drag.axis && Math.max(Math.abs(deltaX), Math.abs(deltaY)) > 7) {
        if (Math.abs(deltaY) > Math.abs(deltaX)) { drag = null; return; }
        drag.axis = 'horizontal'; book.classList.add('is-dragging'); book.setPointerCapture(event.pointerId);
      }
    ...
    const finishDrag = (event) => {
      if (!drag) return;
      const { page, delta, width, locked, axis } = drag;
      drag = null; book.classList.remove('is-dragging');
      if (locked) {
        const rect = book.getBoundingClientRect();
        if (!axis && event.clientX < rect.left + rect.width / 2) restore();
        return;
      }
      if (Math.abs(delta) > width * .2) { dismiss(delta < 0 ? -1 : 1, true); return; }
      if (!axis || Math.abs(delta) < 8) {
        const rect = book.getBoundingClientRect();
...
async function loadVelvetPodcasts() {
  ...
  try {
    ...
    podcasts.forEach((podcast, index) => {
      ...
      link.href = releaseId ? `https://velvtmusic.vercel.app/Release?id=${encodeURIComponent(releaseId)}&song=${encodeURIComponent(podcast.id)}&autoplay=1` : 'https://velvtmusic.vercel.app/';
...
async function renderAdmin(resource) {
  ...
  try {
    ...
    const magazineAdsPanel = magazineSettings ? magazineAdsPanelHtml(magazineSettings) : '';
    const bindMagazineCopy = () => {
      bindMagazineAds(section);
...
function renderMagazineAds(settings = {}) {
  ...
  [1, 2, 3, 4].forEach((n) => {
    ...
    const title = String(settings[`ad${n}Title`] || '');
    const text = String(settings[`ad${n}Text`] || '');
    const link = String(settings[`ad${n}Link`] || '');
    const active = String(settings[`ad${n}Active`] ?? '1') !== '0' && Boolean(image || title);
    card.hidden = !active;
    if (!active) return;
    shown += 1;
...
function magazineAdsPanelHtml(settings = {}) {
  const block = (n) => {
    const image = String(settings[`ad${n}Image`] || '');
    return `<fieldset class="vv-ad-edit"><legend>ANÚNCIO ${n}</legend>
      ...
      <label>Link (opcional)<input name="ad${n}Link" type="url" placeholder="https://" value="${escapeHtml(settings[`ad${n}Link`] || '')}"></label>
...
function bindMagazineAds(section) {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      await requestApi('/api/admin/settings', { method: 'PUT', body: JSON.stringify(values) });
      renderMagazineAds(values);
      notify('Anúncios salvos.');
    } catch (error) {
      notify(error.message || 'Não foi possível salvar os anúncios.', 'error');
    } finally {
